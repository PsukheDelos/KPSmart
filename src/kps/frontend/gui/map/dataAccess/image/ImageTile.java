...
package kps.frontend.gui.map.dataAccess.image;
 ...
 * 
 * @see kps.frontend.gui.map.omGraphics.OMRaster
 * @see kps.frontend.gui.map.omGraphics.OMRasterObject
 */
public class ImageTile extends OMScalingRaster implements Serializable {
    ...
    protected boolean shouldFetchForProjection(Projection proj) {
        ...
        float imageScale = kps.frontend.gui.map.proj.ProjMath.getScale(anchor1, anchor2, proj);
    ...
    protected boolean updateImageForProjection(Projection proj) {
        ...
        if (imageDecoder != null) {
            ...
            if (shouldFetchForProjection(proj)) {
                ...
                if (realSelection == null) {
                    if (getFillPaint() == kps.frontend.gui.map.omGraphics.OMColor.clear) {
                ...
                if (getFillPaint() == DEFAULT_NON_CLEAR_FILL_PAINT) {
                    setFillPaint(kps.frontend.gui.map.omGraphics.OMColor.clear);
                }
                setFillPaint(kps.frontend.gui.map.omGraphics.OMColor.clear);
