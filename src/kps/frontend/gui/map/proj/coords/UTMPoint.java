...
import kps.frontend.gui.map.proj.Ellipsoid;
...
public class UTMPoint {
    ...
    public boolean equals(Object obj) {
        if (obj instanceof UTMPoint) {
            ...
            return northing == pnt.northing && easting == pnt.easting && zone_number == pnt.zone_number
                    && zone_letter == pnt.zone_letter;
        }
        return false;
    }
    ...
    public String toString() {
        return "UTMPoint[zone_number=" + zone_number + ", easting=" + easting + ", northing=" + northing + ", hemisphere="
                + zone_letter + "]";
    }
        ...
        M =
                a
                        * ((1 - eccSquared / 4 - 3 * eccSquared2 / 64 - 5 * eccSquared3 / 256) * LatRad
                                ...
                                + (15 * eccSquared2 / 256 + 45 * eccSquared3 / 1024) * Math.sin(4 * LatRad) - (35 * eccSquared3 / 3072)
                                * Math.sin(6 * LatRad));

        double UTMEasting =
                (k0
                        * N
                        * (A + (1 - T + C) * A * A * A / 6.0d + (5 - 18 * T + T * T + 72 * C - 58 * eccPrimeSquared) * A * A * A
                                * A * A / 120.0d) + 500000.0d);

        double UTMNorthing =
                (k0 * (M + N
                        * Math.tan(LatRad)
                        * (A * A / 2 + (5 - T + 9 * C + 4 * C * C) * A * A * A * A / 24.0d + (61 - 58 * T + T * T + 600 * C - 330 * eccPrimeSquared)
                                * A * A * A * A * A * A / 720.0d)));
        if (!isNorthern) {
    ...
    public int hashCode() {
        ...
        result = HashCodeUtil.hash(result, northing);
        result = HashCodeUtil.hash(result, easting);
        result = HashCodeUtil.hash(result, zone_number);
        result = HashCodeUtil.hash(result, zone_letter);
        return result;
    }
     ...
     *         LatLonPoint, it will be returned as well, if successful.
     */
    public static LatLonPoint UTMtoLL(Ellipsoid ellip, double UTMNorthing, double UTMEasting, int ZoneNumber, boolean isNorthern,
                                      LatLonPoint llpoint) {
