...
public class SimpleEmptyTileHandler
   ...
   protected static Logger logger = Logger.getLogger("kps.frontend.gui.map.dataAccess.mapTile.EmptyTileHandler");
    ...
    * (non-Javadoc)
    *
    * @see kps.frontend.gui.map.dataAccess.mapTile.EmptyTileHandler#getOMGraphicForEmptyTile (java.lang.String, int, int,
    * int, kps.frontend.gui.map.dataAccess.mapTile.MapTileCoordinateTransform, kps.frontend.gui.map.proj.Projection)
    */
   ...
   protected BufferedImage createTileImageFromDrawingAttributes(DrawingAttributes da) {
      ...
      if (da != null) {
         ...
         da.setTo(rect);
         rect.generate(new kps.frontend.gui.map.proj.Mercator(new LatLonPoint.Double(), 100000, TILE_SIZE, TILE_SIZE));
