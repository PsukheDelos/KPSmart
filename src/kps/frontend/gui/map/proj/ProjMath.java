...
public final class ProjMath {
     ...
     * @see kps.frontend.gui.map.proj.coords.LatLonPoint#normalizeLatitude(double)
     */
     ...
     * (int i = 0; i < 100; i++) { lat =
     * kps.frontend.gui.map.LatLonPoint.normalize_latitude(
     * (float)Math.random()*LAT_DEC_RANGE); lon =
     * kps.frontend.gui.map.LatLonPoint.wrap_longitude(
