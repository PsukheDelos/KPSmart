...
package kps.frontend.gui.map.layer.location.csv;
...
import kps.frontend.gui.map.layer.location.AbstractLocationHandler;
import kps.frontend.gui.map.layer.location.Location;
import kps.frontend.gui.map.layer.location.LocationCBMenuItem;
import kps.frontend.gui.map.layer.location.LocationHandler;
import kps.frontend.gui.map.layer.location.LocationLayer;
import kps.frontend.gui.map.layer.location.LocationMenuItem;
import kps.frontend.gui.map.layer.location.URLRasterLocation;
import kps.frontend.gui.map.omGraphics.OMGraphic;
import kps.frontend.gui.map.omGraphics.OMGraphicList;
import kps.frontend.gui.map.util.CSVTokenizer;
import kps.frontend.gui.map.util.DataOrganizer;
import kps.frontend.gui.map.util.PropUtils;
import kps.frontend.gui.map.util.quadtree.QuadTree;
 ...
 *       
 *       csvlocationhandler.class=kps.frontend.gui.map.layer.location.csv.CSVLocationHandler
...
public class CSVLocationHandler
    ...
    public Properties getPropertyInfo(Properties list) {
        ...
        list.put("class" + ScopedEditorProperty, "kps.frontend.gui.map.util.propertyEditor.NonEditablePropertyEditor");
        list.put(LocationFileProperty, "URL of file containing location information.");
        list.put(LocationFileProperty + ScopedEditorProperty, "kps.frontend.gui.map.util.propertyEditor.FUPropertyEditor");
        list.put(eastIsNegProperty, "Flag to note that negative latitude are over the eastern hemisphere.");
        list.put(eastIsNegProperty + ScopedEditorProperty, "kps.frontend.gui.map.util.propertyEditor.YesNoPropertyEditor");
    ...
    protected QuadTree createData() {
        ...
        try {
            ...
            throw new kps.frontend.gui.map.util.HandleError(ioe);
        } catch (ArrayIndexOutOfBoundsException aioobe) {
            throw new kps.frontend.gui.map.util.HandleError(aioobe);
        } catch (NumberFormatException nfe) {
            throw new kps.frontend.gui.map.util.HandleError(nfe);
        } catch (ClassCastException cce) {
            logger.warning("Problem reading entries in " + locationFile + ", check your index settings, first column = 0.");
            throw new kps.frontend.gui.map.util.HandleError(cce);
        } catch (NullPointerException npe) {
            logger.warning("Problem reading location file, check " + locationFile);
            throw new kps.frontend.gui.map.util.HandleError(npe);
        } catch (java.security.AccessControlException ace) {
            throw new kps.frontend.gui.map.util.HandleError(ace);
        }
     ...
     * 
     * @see
     * kps.frontend.gui.map.layer.location.LocationHandler#getItemsForPopupMenu(com
